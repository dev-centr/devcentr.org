...
* Build job: Ubuntu, Node 22, pnpm 9; checks out sibling repos for changelog ingest, stack-advisor SDL, and agent-rules bootstrap profiles; `pnpm install --no-frozen-lockfile` then `pnpm run build`. Static build output is in `.output/public`. A `.nojekyll` file is added there so GitHub Pages serves `_build` and `_server` correctly. Post-build, CI asserts `skills/index.html` and `catalog/bootstrap-profiles.json` exist.
