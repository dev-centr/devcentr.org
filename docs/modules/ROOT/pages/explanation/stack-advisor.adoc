...
* Numbered segments with `/` connectors; green light when a level has a selection; **×** (left of slash) removes a level from the path.
* **+** (right of slash) inserts a missing catalog level between neighbors; circular **+** outside the strip appends.
* Off-path levels do not constrain advice. **Reset to sample** restores the full preload.
