...
| `/templates`
| Org bootstrap profile selector (SDL from `agent-rules` / `bootstrap-org`)
...
* `agent-rules/` — bootstrap-org SDL catalog for `/templates`
...
* `src/components/` — shared UI (header, footer, diagrams, advisor widget, bootstrap profile selector)
