...
For `/changelog`, `/toolchain-advisor`, and `/templates` with live sibling data, clone checkouts beside this repo:
...
* **Templates page empty** — run `pnpm sync-bootstrap-profiles` with `agent-rules` checked out; verify `public/catalog/bootstrap-profiles.json` exists (gitignored, not committed).
* **CI fails `templates/index.html`** — Vinxi does not emit a directory index for client-only routes; `scripts/spa-fallback.mjs` must copy the SPA shell into `.output/public/templates/`.
