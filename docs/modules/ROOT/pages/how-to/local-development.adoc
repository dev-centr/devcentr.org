...
== Full fidelity (changelog + stack advisor + templates)

For `/changelog`, `/stack-advisor`, and `/skills` with live sibling data, clone checkouts beside this repo:

[source,bash]
----
cd ..
git clone https://github.com/dev-centr/stack-advisor.git
...
| `pnpm sync-advisor`
| Copy and compile stack-advisor SDL → `public/catalog/`
...
* **Advisor page empty** — run `pnpm sync-advisor` with `stack-advisor` checked out; verify `public/catalog/advisor.json` exists.
...
* **New idea page 404 on GitHub Pages** — add `/ideas/<slug>` to `app.config.ts` prerender routes; confirm `scripts/spa-fallback.mjs` ran in `postbuild`. See xref:explanation/apps-catalog.adoc[Apps catalogue].
