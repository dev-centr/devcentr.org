...
export function CatalogGate(props: {
  eyebrow: string;
  title: string;
  intro: string;
  panes: CatalogGatePane[];
}) {
  ...
  return (
    <main class="flex min-w-0 flex-1 flex-col overflow-x-clip">
      <div class="mx-auto w-full max-w-6xl px-6 pt-6 md:px-10">
        <p class="eyebrow">{props.eyebrow}</p>
        <h1 class="mt-3 font-display text-4xl font-semibold tracking-tight md:text-5xl">{props.title}</h1>
        <p class="mt-3 max-w-xl text-muted-foreground">{props.intro}</p>
      </div>
            ...
            return (
              <a
                ...
                classList={{
                  ...
                  "apps-gate-pane--skew": index() < props.panes.length - 1,
                }}
